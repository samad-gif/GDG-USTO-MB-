...
let userAnswers = [];
let score = 0;
let skippedIndices = []; // track intentionally skipped questions
...
// Floating nav buttons (right-side quick links) — make them act like the top nav
...
floatBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        // Update active in top nav
        navLinks.forEach(l => l.classList.remove('active'));
        const corresponding = document.querySelector(`.nav-link[href="#${targetId}"]`);
        ...
        // Close mobile menu and reset if returning home
        navMenu.classList.remove('active');
        if (targetId === 'home') {
            resetQuiz();
        }
...
// Skip current question: mark as skipped and go to next unanswered or next available
if (typeof skipBtn !== 'undefined') {
    skipBtn.addEventListener('click', (e) => {
        e.preventDefault();
        const questions = quizData[currentBranch].questions;
        if (!skippedIndices.includes(currentQuestionIndex)) skippedIndices.push(currentQuestionIndex);

        // Find next unanswered after current
        let next = null;
        for (let j = currentQuestionIndex + 1; j < questions.length; j++) {
            if (userAnswers[j] === null) { next = j; break; }
        }
        // otherwise find any unanswered
        if (next === null) {
            for (let j = 0; j < questions.length; j++) {
                if (userAnswers[j] === null && j !== currentQuestionIndex) { next = j; break; }
            }
        }
        // otherwise go to first skipped that's not current
        if (next === null) {
            for (let idx of skippedIndices) { if (idx !== currentQuestionIndex) { next = idx; break; } }
        }
        ...
        if (next !== null) {
            ...
            feedback.innerHTML = `<i class="fas fa-info-circle"></i> Question ignorée. Vous pouvez y répondre maintenant.`;
        } else {
            feedback.classList.remove('hidden');
            feedback.classList.remove('correct','wrong');
            feedback.classList.add('warning');
            feedback.innerHTML = `<i class="fas fa-info-circle"></i> Aucune autre question non répondue.`;
        }
    });
}
...
function initQuiz() {
    ...
    const questions = quizData[currentBranch].questions;
    const unanswered = userAnswers.map((a,i)=> a === null ? i : -1).filter(i=> i >= 0);
    if (unanswered.length > 0) {
        ...
        feedback.classList.remove('hidden');
        feedback.classList.add('warning');
        feedback.innerHTML = `<i class="fas fa-info-circle"></i> Vous avez ${unanswered.length} questions non répondues. Elles seront rouvées maintenant.`;
    } else {
        showResults();
    }
});
...
function showQuestion() {
    ...
    // Skip available at all times while quiz is open
    if (typeof skipBtn !== 'undefined') skipBtn.classList.remove('hidden');
}
...
function selectOption(optionIndex) {
    ...
    // If user answered a previously skipped question, remove it from skippedIndices
    const skipPos = skippedIndices.indexOf(currentQuestionIndex);
    if (skipPos !== -1) skippedIndices.splice(skipPos, 1);
...
function showNextQuestion() {
    const questions = quizData[currentBranch].questions;
    // If current not answered, ask user to answer or skip
    if (userAnswers[currentQuestionIndex] === null) {
        feedback.classList.remove('hidden');
        feedback.classList.remove('correct','wrong');
        feedback.classList.add('warning');
        feedback.innerHTML = '<i class="fas fa-exclamation-circle"></i> Vous devez répondre ou cliquer sur <strong>Passer</strong> pour ignorer cette question.';
        return;
    }
    ...
    if (currentQuestionIndex < questions.length - 1) {
        ...
        // At end — if any unanswered, go to first unanswered
        const unanswered = userAnswers.map((a,i)=> a === null ? i : -1).filter(i=> i >= 0);
        if (unanswered.length > 0) {
            currentQuestionIndex = unanswered[0];
            showQuestion();
            feedback.classList.remove('hidden');
            feedback.classList.add('warning');
            feedback.innerHTML = `<i class="fas fa-info-circle"></i> Vous avez ${unanswered.length} questions non répondues. Répondez-les ou cliquez sur <strong>Passer</strong>.`;
        }
    }
}
